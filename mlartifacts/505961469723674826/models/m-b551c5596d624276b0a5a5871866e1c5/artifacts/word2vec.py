...
# Define the model path and recipe path
...
class Word2VecModel(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context):
        ...
        recipe_vector_means = []
        self.data = pd.read_pickle(recipe_path)
    ...
    def predict(self, context, model_input: list[list[int]]) -> str:
        ...
        user_vec = np.zeros(self.recipe_vector_means.shape[1]) + np.sum(self.recipe_vector_means[list(liked_idx)], axis=0) - (0.3 * np.sum(self.recipe_vector_means[list(disliked_idx)], axis=0))
...
mlflow.models.set_model(Word2VecModel(10))
