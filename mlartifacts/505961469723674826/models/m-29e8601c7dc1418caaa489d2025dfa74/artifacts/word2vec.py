...
class Word2VecModel(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input: list[list[int]]) -> str:
        ...
        like_step = context.artifacts["like_step"]
        dislike_step = context.artifacts["dislike_step"]
        ...
        user_vec = np.zeros(self.recipe_vector_means.shape[1]) + (like_step * np.sum(self.recipe_vector_means[list(liked_idx)], axis=0)) - (dislike_step * np.sum(self.recipe_vector_means[list(disliked_idx)], axis=0))
